...
The smoothness benchmark adds browser `requestAnimationFrame` interval measurements while the example camera-stream path is active, and also records hierarchy expansion, node selection, point rendering, and total stream-update timing so point-budget tuning can be compared with repeatable frame-time data.
